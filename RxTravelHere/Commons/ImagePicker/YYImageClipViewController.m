...
- (void)initView {
    ...
    if (self.originalImage.size.height > self.originalImage.size.width) {
        ...
        oriHeight = self.originalImage.size.height / self.originalImage.size.width * oriWidth;
        oriX = self.cropFrame.origin.x;
        oriY = self.cropFrame.origin.y - (oriHeight - self.cropFrame.size.height)*0.5;
    } else {
        oriHeight = self.cropFrame.size.height;
        oriWidth = self.originalImage.size.width / self.originalImage.size.height * oriHeight;
        oriX = (self.cropFrame.size.width-oriWidth)*0.5;
    ...
    // Add
    if (self.isCycle) {
        CGFloat width = MIN(self.cropFrame.size.width, self.cropFrame.size.height);
        self.ratioView.layer.cornerRadius = width * 0.5;
...
- (void)overlayClipping
{
    ...
    if (self.isCycle) {
        ...
        maskLayer.path = pOtherPath.CGPath;
        UIBezierPath *pPath = [UIBezierPath bezierPathWithOvalInRect:self.ratioView.frame];
