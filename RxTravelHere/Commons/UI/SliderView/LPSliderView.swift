...
public class LPSliderView: UIView, UIScrollViewDelegate {
    ...
    /// 小红点数组
    private var redViews = [RedPointView]() {
        didSet {
            if redPoints.count == redViews.count {
                for i in 0..<redPoints.count {
                    redViews[i].count = redPoints[i]
                }
            }
        }
    }
    public var redPoints = [Int]() {
        didSet {
            if redPoints.count == redViews.count {
                for i in 0..<redPoints.count {
                    redViews[i].count = redPoints[i]
    ...
    public override func draw(_ rect: CGRect) {
        ...
        if redPoints.count > 0 {
            assert(titles.count == redPoints.count, "标题个数和小红点个数不相等！")
        }
    ...
    private func maketopView() {
        ...
        for i in 0 ..< subViewCount {
            ...
            // 添加小红点
            if redPoints.count > 0 {
                let pointW: CGFloat = redPointW
                let pointX: CGFloat = btnX + 0.75 * btnW
                let pointY: CGFloat = 10
                let point = RedPointView(frame: CGRect(x: pointX, y: pointY, width: pointW, height: pointW))
                addSubview(point)
                
                redViews.append(point)
                point.count = redPoints[i]
            }
        }
